...
# Main API function for object detection using camera
...
def find_item_in_scene():
    ...
    if args.scene:
        scene_img = utils.capture_camera_image()
        if scene_img is None:
            print("No scene image captured")
            return
    base_interpreter, base_input_details, base_output_details = feature_extractor.load_tflite_model(config.MODEL_TFLITE_PATH)
    ...
    scene_img = utils.load_image_webcam_uint8(scene_img, scale=config.DOWNSCALE_FACTOR)    
