...
def check_font(font=FONT, progress=False):
    """Ensures the specified font exists by using a local copy if available,
    otherwise downloads it from Ultralytics assets.
    """
    font = Path(font)
    file = CONFIG_DIR / font.name
    # Define the path where you manually placed your font locally.
    local_font_path = Path("data/fonts") / font.name

    # First, check if the local copy exists
    if local_font_path.exists():
        LOGGER.info(f"Found local font at {local_font_path}. Copying to {file}...")
        file.write_bytes(local_font_path.read_bytes())
    # Otherwise, if neither the passed path nor the CONFIG_DIR file exist, download the font.
    elif not font.exists() and not file.exists():
        url = f"https://github.com/ultralytics/assets/releases/download/v0.0.0/{font.name}"
        LOGGER.info(f"Downloading {url} to {file}...")
        torch.hub.download_url_to_file(url, str(file), progress=progress)
    else:
        LOGGER.info(f"Font already exists at {file} or {font}.")
